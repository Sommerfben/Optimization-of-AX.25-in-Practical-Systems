...
N_MAX = 50            #max frames per packet
n_MAX = 200          #max number of info bytes
BER = 0.01              #10^-3
...
def stat_analysis():
    ...
    N_values = value_extractor(results, 1,1000)
    n_values = value_extractor(results, 0,1000)
    print(N_values)
    plt.bar(N_values, n_values, align='center', alpha=0.5)
    plt.ylabel('n values')
    plt.xlabel('N values')
    plt.title('Optimum Values of n and N')
